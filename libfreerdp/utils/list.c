...
void* list_remove_debug(LIST* list, void* data,const char * function)
{
	corrib_syslog(LOG_DEBUG,"%s:Called by %s\n ",__func__,function);

	LIST_ITEM* item;
#ifdef TRACK_CALLER
    unsigned int retAddr;
    asm("mov %0, lr" : "=r" (retAddr) : );
#endif
	//sh_log_print_trace ();
    sh_log_print_trace ();
	corrib_syslog(LOG_DEBUG,"%s:\n",__func__);
	item = list_item_find(list, data);
	if (item != NULL)
	{
		if (item->prev != NULL)
