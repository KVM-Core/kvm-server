...
BOOL freerdp_check_file_exists(const char* file)
{
...
BOOL freerdp_check_file_exists_and_delete(const char* file)
{
	struct stat stat_info;
...
char* freerdp_get_config_path(rdpSettings* settings)
{
	...
	settings->config_path = (char*) xmalloc(strlen(settings->home_path) + sizeof(FREERDP_CONFIG_DIR) + 2,__func__);
...
char* freerdp_get_current_path(rdpSettings* settings)
{
	if (settings->current_path == NULL)
		settings->current_path = xgetcwd(NULL, 0,__func__);
...
char* freerdp_construct_path(char* base_path, char* relative_path)
{
	...
	path = xmalloc(length + 1,__func__);
...
char* freerdp_append_shared_library_suffix(char* file_path)
{
	...
	if (file_path_length >= shared_lib_suffix_length)
	{
		...
		{
			path = xmalloc(file_path_length + shared_lib_suffix_length + 1,__func__);
...
char* freerdp_get_parent_path(char* base_path, int depth)
{
	...
	path = (char*) xmalloc(length + 1,__func__);
...
void freerdp_remove_file(const char *fName)
{
	if (freerdp_check_file_exists(fName)) {
		int ret = remove(fName);

		if (ret == 0) {
			printf("Dfreerdp - Deleted %s\n", fName);
		} else {
			printf("Dfreerdp - Unable to Delete %s\n", fName);
		}
	}
}
